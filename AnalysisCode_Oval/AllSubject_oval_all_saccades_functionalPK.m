...
subjects = {'OvalEyeMovement-subject10'};
...
for sub=1:num_sub
    ...
    [params, hprs_used, data, accuracy, num_image, mean_oval_signal,...
        num_trial, num_trial_sanity, acc_evidence, choice_in_favor, random_landing,...
        acc_evidence_sanity, choice_in_favor_sanity, random_landing_sanity,...
        fixations_num, eyelink, saccade_code, fixation_dist,...
        saccades_used, saccades_used_sanity, saccade_dist_mid,...
        logits, spatial_pk, temporal_fixation_pk, temporal_duration_pk, temporal_pk_across_trials]...
        = analysis_across_allsaccades_functionalPK(subjects{sub}, expt_type, boots,fix_cond,...
        saccade_types, saccade_types_sanity, fix_cluster_dist, peripheryPKbound, sanity_bnd, hprs_ridge);
    
    best_hprs(sub) = hprs_used;
...
for sub=1:num_sub
    ...
    ax.XAxis.FontSize = 20;
    ax.YAxis.FontSize = 20;
    title('Fixation based weights','Fontsize',20);
    ...
    ax.XAxis.FontSize = 20;
    ax.YAxis.FontSize = 20;
    title('Time stamp based weights','Fontsize',20)
    ...
    ax.XAxis.FontSize = 20;
    ax.YAxis.FontSize = 20;
    title('Fixation + time stamp based weights','Fontsize',20);
...
for sub=1:num_sub
    ...
    max_fx_pk = max(max_fx_pk,max(temporal_fixation_kernel{sub}(:)));
    min_fx_pk = min(min_fx_pk,min(temporal_fixation_kernel{sub}(:)));
    max_dur_pk = max(max_dur_pk,max(temporal_duration_kernel{sub}(:)));
    min_dur_pk = min(min_dur_pk,min(temporal_duration_kernel{sub}(:)));
    max_comb_pk = max(max_comb_pk,max(temporal_kernel_across_trials{sub}(:)));%./scaling(sub,1)));
...
for bn=2:length(saccade_types)
    ...
    for sub=1:num_sub
        ...
        if sub==1
            ...
            errorbar(1:saccade_types(bn), prob_choice_favor_random{bn}(sub,:),prob_choice_favor_random_err{bn}(sub,:),'-or','LineWidth',2);
        end
        hold on;
        if sub==3 || sub==8
            xlabel('Saccade length order (low to high)','Fontsize',30);
        end
        if sub==1 || sub==6
    ...
    sgtitle(['Bias vs saccade lengths with ' num2str(saccade_types(bn)) ' bins for all trials'],'fontsize',30);
end
...
for ev_bn=1:length(evidence_types)
    ...
    sgtitle(['Bias vs evidence accumulated with ' num2str(evidence_types(ev_bn)) ' bins for all trials'],'fontsize',30);
end
