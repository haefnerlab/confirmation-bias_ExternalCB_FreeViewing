function [mean_bin_index,prob] = compute_extCB_ideal_closest(data,mean_oval_signal,split_parameter,bin,boot_n,pk,choose_abs_PK)
% initialize
[ParentFolderPath] = fileparts(pwd);
datadir = fullfile(ParentFolderPath, '/RawData');
%if we need to threshold anytime.
...
for trial=1:size(mean_oval_signal,1)
    ...
    for i=1:split_parameter-1
        weight = temporal_kernel((i-1)*num_image+1);
        if choose_abs_PK==1
            weight(weight<0) = 0;
        end
        if i==1
