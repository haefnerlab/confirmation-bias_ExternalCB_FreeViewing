function [params_boot,data,accuracy,num_image,performance_bnd,oval_sig,...
    num_trial,num_trial_sanity,num_trial_thresh,...
    acc_evidence,choice_in_fav,weights_on_saccades,sp_weights_on_saccades,random_landing,...
    acc_evidence_sanity,choice_in_fav_sanity,random_landing_sanity,...
    acc_evidence_thresh,choice_in_fav_thresh,weights_on_saccades_thresh,sp_weights_on_saccades_thresh,random_landing_thresh,...
    fixations_per_trial,eyelink,saccade_code,fixation_dist,saccades_used,saccades_used_sanity,saccades_used_thresh,saccade_dist_mid,...
    log_bernoulli, spatial_pk, temporal_fixation_pk, temporal_duration_pk, temporal_pk_across_trials]...
    = analysis_across_allsaccades_functionalPK_SingleHyprs(subjectID, expt_type, boot_n,...
    fix_cond,saccade_types,saccade_types_sanity,saccade_types_thresh,fix_cluster_dist,peripheryPKbound,sanity_bnd,boots_thresh,threshold_performance,hprs_ridge,standardize)
...
saccade_dist_edges_thresh = linspace(min(fixation_dist_all),max(fixation_dist_all),saccade_types_thresh+1);
...
for j = 1:boot_n
    ...
    [params_boot(j,:)] = CustomRegression.PsychophysicalKernelwithlapse_functionalPK(squeeze(signal_result(j,:,:)), squeeze(choice_result(j,:)), squeeze(ordered_oval_distance_result(j,:,:,:)), squeeze(fixation_onset_time_stamps_result(j,:,:)), max_fix, hprs_ridge, standardize);
end

disp('Computing bias in saccade...');
%Obtaining bias in saccade
for ss=1:length(saccade_types)
    [acc_evidence{ss},choice_in_fav{ss},weights_on_saccades{ss},sp_weights_on_saccades{ss},random_landing{ss},saccades_used{ss}] = ...
...
floor_boots = getBootstrapTheshold(data,threshold_performance(1),boots_thresh);
thresh_boots = getBootstrapTheshold(data,threshold_performance(2),boots_thresh);
floor = mean(floor_boots);
thresh = mean(thresh_boots);
performance_bnd = [floor thresh];

disp('Computing bias in saccade for threshold trials ...');
%Obtaining bias in saccade
[acc_evidence_thresh,choice_in_fav_thresh,weights_on_saccades_thresh,sp_weights_on_saccades_thresh,random_landing_thresh,saccades_used_thresh,num_trial_thresh] = ...
    compute_extCB_all_saccades_thresh_functionalPK(data,fixations_per_trial,sorted_oval_all,...
    saccade_dist_edges_thresh,ordered_oval_distance, fixation_onset_time_stamps, ...
    params_boot,fixation_dist,peripheryPKbound,performance_bnd);
