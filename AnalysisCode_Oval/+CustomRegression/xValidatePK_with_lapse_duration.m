function [best_hprs, log_likelihoods] = xValidatePK_with_lapse_duration(data, responses, frames, num_im, hpr_ridge, hpr_ar1, hpr_curvature, standardize, folds, learn_num_PK)
...
%         hpr_ridge(iRidge), hpr_ar1(iAR1), hpr_curvature(iCurve), 0);
    [params, ~, ~, ~, ~, ~] = ...
    CustomRegression.PsychophysicalKernelwithlapse_duration(data(dataIdx, :), responses(dataIdx), frames, num_im, learn_num_PK,...
        hpr_ridge(iRidge), hpr_ar1(iAR1), hpr_curvature(iCurve), 0);
    log_likelihoods(ii) = bernoulli_log_likelihood(data(splitStart(iFold):splitEnd(iFold), :), ...
        responses(splitStart(iFold):splitEnd(iFold)), params);
end
...
function LL = bernoulli_log_likelihood(data, responses, params)
...
lapse = params(end)^2;
logits = data * weights(:);
log_bernoulli = -1*(-log(exp(logits(:)).*(responses*(1-lapse)+0.5*lapse) + ((1-responses)*(1-lapse)+0.5*lapse)) + log(1 + exp(logits(:))));
