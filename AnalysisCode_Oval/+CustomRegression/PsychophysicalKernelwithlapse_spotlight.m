function [weights, postVal, errors, map_ridge, map_ar1, map_curvature] = ...
    PsychophysicalKernelwithlapse_spotlight(data, responses, num_frames,  hpr_ridge, hpr_ar1, hpr_curvature, standardize)
...
function [optim_weights, postVal, errors] = do_fit(data, responses, frames, hpr_ridge, hpr_ar1, hpr_curvature)
% Perform fitting at a single setting of hyperparameters / at a single
% point in the grid search.

[~, p] = size(data);
break_points = [];
for rr=1:frames%fix(p/frames)
    break_points = [break_points rr*fix((p-1)/frames)];
end
break_points = [break_points p-1 p]; % don't smooth onto bias term
...
options = optimset('MaxIter', 1000000, 'MaxFunEvals', 1000000,'Display', 'off','Algorithm', 'trust-region');
[optim_weights, negPostVal, ~, ~, ~, hessian] = fminunc(@neg_log_posterior, zeros(p+1,1), options);
