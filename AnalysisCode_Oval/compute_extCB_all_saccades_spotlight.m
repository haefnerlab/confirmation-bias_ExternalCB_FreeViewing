function [mean_bin_index,prob,mean_bin_index_random,prob_random,acc_evidence,choice_in_fav,random_landing,saccades_used] = compute_extCB_all_saccades_spotlight(data,fixations_per_trial,oval_signal,bin,boot_n,pk,saccade_dist,saccade_length,peripheryPKbound)
% initialize
...
for trial=1:size(oval_signal,1)
    for i=1:fixations_per_trial(trial)-1
        ...
        for k=1:size(saccade_length,1)
            if saccade_dist{trial}(i)<=saccade_length{k}{trial}(i)
