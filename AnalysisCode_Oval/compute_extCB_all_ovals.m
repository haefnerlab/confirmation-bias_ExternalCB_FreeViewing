function [mean_bin_index,prob,acc_evidence] = compute_extCB_all_ovals(data,mean_oval_signal,split_parameter,bin,boot_n,pk,choose_abs_PK)
% initialize
...
for trial=1:size(mean_oval_signal,1)
    for i=1:split_parameter-1
        weight = temporal_kernel(1:i*num_image);
        if choose_abs_PK==1
            weight(weight<0) = 0;
        end
        acc_evidence(trial,i) = bias + sum(mean_oval_signal(trial,1:i*num_image).*weight); %pass on PK and .*pk(1:i*num_image)
    end
end
